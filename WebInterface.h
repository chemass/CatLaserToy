...
<body>
    ...
    <script>
        ...
        socket.onmessage = (event) => {
            ...
            try {
                ...
                if (data.type === 'position') {
                    // Only update pointer position if not locally controlled
        ...
        let isLocallyControlled = false; // Flag to disable WebSocket position updates during local control
        let lastSendTime = 0;
        ...
        box.addEventListener('mouseup', e => { 
            isDragging = false; 
            // Clear local control flag after a short delay to avoid immediate position updates
            setTimeout(() => { isLocallyControlled = false; }, 100);
        });
        ...
        document.querySelectorAll('.fine-control').forEach(control => {
            control.addEventListener('click', (e) => {
                // Set local control flag during fine control
                isLocallyControlled = true;
                ...
                // Clear local control flag after a short delay
                setTimeout(() => { isLocallyControlled = false; }, 100);
            });
        });
    ...
    // Note: The actual C++ code to serve this HTML content via a web server on the ESP32
    // would be implemented in the corresponding .cpp file, using appropriate libraries
    // such as ESPAsyncWebServer or WebServer.
    
    // The HTML content includes:
    // - A responsive design with CSS for styling buttons, control box, and layout.
    // - JavaScript for WebSocket communication, UI interactions, and control logic.
    // - Buttons for laser toggle, setup mode, storing points, and selecting patterns.
    // - A control box for manual laser positioning with touch/mouse support.
    // - Setup mode with corner indicators and fine control borders.
    // - Tab navigation between Control and Patterns sections.
